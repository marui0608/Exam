import os,xlrd,datetime,random
    ...
    if request.method == 'POST':
        ...
        if bank_name and bank_type and bank_template:
            ...
            if not bank:
                ...
                # 添加到题库录入表
                bank_data = BankXlsx(bank_name=bank_name, bank_type=bank_type, bank_xlsx=bank_template,bank_nums=table.nrows-1,bank_chio=num1,bank_chios=num2,bank_judge=num3,bank_gap=num4,bank_person=0,bank_bs=0)
                bank_data.save()
                ...
                for i in range(1, table.nrows):
                    ...
                    if ti_type == '单选题':
                        ...
                        if question and answer and A and B and C and D:
                            cun1 = Ques_choice(question=question,answer=answer,choiceA=A,choiceB=B,choiceC=C,choiceD=D,score=5,choi_id_id=bank1.id)
                            cun1.save()
                        else:
                            pass
                    elif ti_type == '多选题':
                        ...
                        D = table.row_values(i)[6]
                        if question and answer and A and B and C and D:
                            cun2 = Ques_choices(question=question,answer=answer,choiceA=A,choiceB=B,choiceC=C,choiceD=D,score=5,chois_id_id=bank1.id)
                            cun2.save()
                        else:
                            pass
                    elif ti_type == '判断题':
    ...
    if request.method == 'POST':
        ...
        if type and bank:
            if name and num and score and starttime and endtime and times:
                g = Game(game_name=name,game_nums=num,game_score=score,create_time=starttime,end_time=endtime,time_bar=times,game_rule=ruleText,class_game_id=Classify.objects.get(class_name=type).id,bank_game_id=bank_info.id)
                g.save()
                bank_info.bank_bs += 1
                bank_info.save()
            ...
            con = '请选择题库'
    return render(request,'setgames/game.html',{'class_datas':class_datas,'bank_datas':bank_datas,'type_date':type,'bank':bank,'bank_info':bank_info,'con':con})
...
def test_start(request,id):
    ...
    for data1,data2,data3,data4 in zip(choi_data,chois_data,judge_data,gap_data):
    ...
    if request.method == 'POST':
        my_score = 0
        danxuan = request.POST.get('danxuan')
        duoxuan = request.POST.getlist('duoxuan')
        panduan = request.POST.get('panduan')
        tiankong = request.POST.get('tiankong')
        print(danxuan,str(duoxuan),panduan,tiankong)
        for i in choi_data:
            pass

    return render(request,'competition/game.html',{'test_data':test_data,'bank_data':page_data})
...
def rank(request):
    ...
    return render(request,'competition/rank.html',{'game':game,'order':order})
